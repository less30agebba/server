...
public class RedisClient {
    ...
    public String brpop(final String key) {
        String result = redisTemplate.execute((RedisCallback<String>) connection -> {
            RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
            //Note: 0代表一直阻塞数据的获取
            List<byte[]> res = connection.bRPop(0,serializer.serialize(key));
            return serializer.deserialize(res.get(1));
        });
        return result;
    }
    ...
    public String blpop(final String key) {
        String result = redisTemplate.execute((RedisCallback<String>) connection -> {
            RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
            //Note: 0代表一直阻塞数据的获取
            List<byte[]> res = connection.bLPop(0,serializer.serialize(key));
            return serializer.deserialize(res.get(1));
        });
        return result;
    }

    public boolean publish(final String key,final String value){
        return redisTemplate.execute((RedisCallback<Boolean>) connection -> {
            RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
            connection.publish(serializer.serialize(key), serializer.serialize(value));
            return true;
        });
    }

    public void subscribe(final String key,Consumer<String> messageConsumer){

        redisTemplate.execute((RedisCallback<Boolean>)connection -> {
            RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
            connection.subscribe((message, pattern) -> {
                messageConsumer.accept(serializer.deserialize(message.getBody()));
